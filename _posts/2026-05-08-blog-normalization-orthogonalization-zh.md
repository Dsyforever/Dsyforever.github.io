...
这一机制也可以从高维高斯向量的几何直觉来理解。对于两个独立的高斯向量 $u, v \sim \mathcal{N}(0, I_{d_0})$，有
...
也就是说，在期望意义下，不同高斯向量彼此近似正交，而向量与自身的内积则会随着维数增长。这正好对应了这里非对角项与对角项的差别：$(G_{1,t} G_{1,t}^\top)_{ac}$（$a \neq c$）描述的是不同隐藏神经元梯度行之间的相关性，类似于两个不同随机向量的内积，因此不会随着隐藏宽度 $d_1$ 增大而累积，保持在 $\Theta(1)$)；而 $(G_{1,t} G_{1,t}^\top)_{aa}$ 描述的是同一梯度行与自身的内积，类似于向量范数平方，并且在反向传播展开后还会从其余 $d_1 - 1$ 个隐藏神经元那里累积出额外的常数量级贡献，因此主导项表现为 $(d_1 - 1)\kappa_1 = \Theta(d_1)$。

在我们的设定里，隐藏层的宽度 $d_1$ 扮演的正是这种“高维度”角色：初始化时各行权重的高度独立性，会在梯度行之间保留下类似的近正交结构。于是，当 $d_1 \to \infty$ 时，非对角项所对应的相关性弱于对角项所对应的相关性，因此非对角项的增长速度会慢于对角项，进而推动 $G_{1,t} G_{1,t}^\top$ 渐近走向对角占优。
