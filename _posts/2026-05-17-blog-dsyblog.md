...
The source of the gap is exactly the off-diagonal part $E$ of $WW^T$. The entry $E_{i\ell} = \langle W_{i*}, W_{\ell*}\rangle$ measures the correlation between row $i$ and row $\ell$. In high dimension, when $n\gg m$, independent random vectors are nearly orthogonal, so these off-diagonal terms are of relative order $O(1/\sqrt n)$. This is precisely the distinction between Muon and RMNP: Muon **exactly** removes cross-row correlations, while RMNP **ignores** them, but those correlations are already tiny.
...
- Fix $m = 5$
- Let $n$ range over $\{20, 30, 50, 80, 120, 200, 350, 600, 1000, 2000, 5000, 10000, 20000\}$
- Run 500 Monte Carlo samples for each $n$
...
<figure style="margin: 1.4rem auto 1.6rem auto; text-align: center; max-width: 760px;">
  <img src="/images/blog/norm_decay.png" alt="Numerical simulation of how the norm gap between the Muon and RMNP descent directions decays as n increases." style="width: 100%; height: auto; display: block; margin: 0 auto; border-radius: 6px;" />
  <figcaption style="margin-top: 0.55rem; font-size: 0.92rem; line-height: 1.45; color: #5c6675;">Figure 1. The numerical experiment shows that both norms decay at approximately the $n^{-1/2}$ rate.</figcaption>
</figure>

### 6.3 Conclusion

In log-log coordinates, both norms are parallel to the reference line $1/\sqrt n$, verifying

$$\mathbb{E}\|\Delta\|_{\mathrm{op}} = \Theta(1/\sqrt n),\qquad \mathbb{E}\|\Delta\|_F = \Theta(1/\sqrt n).$$

This matches the theoretical prediction exactly.